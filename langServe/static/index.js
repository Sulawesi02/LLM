    ...
    */
    sessionData.messages.forEach(msg => {
        ...
        if (msg.type === "self") {
            ...
            // 创建llm响应消息容器的div元素
            container = document.createElement("div");
            container.className = "llm-msg-container";
            ...
            llmMsg_P.textContent = msg.text;
            llmMsg.appendChild(llmMsg_P);

            // 将llm头像和消息元素添加到容器中
            container.appendChild(llmAvatar);
...
function sendRequest(text) {
    ...
    // 构造数据对象，包括输入的文本和配置
    if (inputText.trim() === '') return; // 如果没有输入内容，不执行发送请求
    const data = {
        input: {
            input: inputText
        },
    ...
    */
    ...
    如果响应状态码为 ok，则继续处理。
    */
    fetch(`http://127.0.0.1:3000/chain/stream_log`, {
        method: 'POST',
        headers: {
